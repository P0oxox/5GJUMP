...
ifaces = ['usb0','wlan0']
def ping_main(): 
    if "usb0" in a["fo"]["main_iface"] or "wlan0" in a["fo"]["main_iface"]:
...
def ping_sec():
    if "usb0" in a["fo"]["sec_iface"] or "wlan0" in a["fo"]["sec_iface"]:
...
def ping_backup():
    if "usb0" in a["fo"]["backup_iface"] or "wlan0" in a["fo"]["backup_iface"]:
...
def choose_status(ping_main, ping_sec, ping_backup):
    ...
    if ping_main is None and ping_sec and ping_backup :
        if "usb0" in a["fo"]["main_iface"] or "wlan0" in a["fo"]["main_iface"]:#if "en" in a["fo"]["main_iface"]:
    ...
    if ping_main and ping_sec is None and ping_backup :
        info_json["main_iface"]["status"] = "Active"
        if "usb0" in a["fo"]["sec_iface"] or "wlan0" in a["fo"]["sec_iface"]:#if "en" in a["fo"]["sec_iface"]:
    ...
    if ping_main and ping_sec is None and ping_backup is None:
        ...
        if "usb0" in a["fo"]["sec_iface"] or "wlan0" in a["fo"]["sec_iface"]:#if "en" in a["fo"]["sec_iface"]:
            info_json["sec_iface"]["status"] = "Fail"
        if "usb0" in a["fo"]["backup_iface"] or "wlan0" in a["fo"]["backup_iface"]:#if "en" in a["fo"]["backup_iface"]:
            info_json["backup_iface"]["status"] = "Fail"
        #print("six")
        if a['fo']["failback"] == True: 
            ifmetric_show = subprocess.getoutput("route -n")
...
path_to_file = "/linux_bridge_PR/5Gjump/failover"
if __name__ == '__main__':

    while True:
        
        info_json = {"main_iface":{"status":''},"sec_iface":{"status":''},"backup_iface":{"status":''}}
        current_config_jsonFile = open('/linux_bridge_PR/5Gjump/current_config.json','r')
