...
def iface_name():
    en_to_eth = {}
    eth_to_en = {}
    en_to_eth["usb0"] = "Mobile Network"
    en_to_eth["wlan0"] = "Wi-Fi"
    # en_to_eth["enp0s9"] = "NR"
...
class supported():
    def find_WAN():
        jsonFile = open('/linux_bridge_PR/5Gjump/iface.json','r')
...
def info():
    jsonFile = open('/linux_bridge_PR/5Gjump/current_config.json','r')
    a = json.load(jsonFile)
    status_ = open('/linux_bridge_PR/5Gjump/failover','r')
    b = json.load(status_)
    if a['mode'] == "fo": 
        result = {}
        result["mode"]="fo"
        if "wlan" in a["fo"]["main_iface"] or "usb" in a["fo"]["main_iface"]:
        ...
        #===========================================================
        if "wlan" in a["fo"]["sec_iface"] or "usb" in a["fo"]["sec_iface"]:
        ...
        #=======================================================
        if "wlan" in a["fo"]["backup_iface"] or "usb" in a["fo"]["backup_iface"]:
